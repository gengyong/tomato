...
class GenerateIntl extends ConsoleCommand
{
    ...
    protected function fetchTerritoryData(OutputInterface $output, $langCode, $requestLangCode, &$translations)
    {
        ...
        try {
            ...
            $regions = array('Country_HK', 'Country_MO', 'Country_TW');
            $motherland = $translations['Intl']['Country_CN'];
            foreach($regions as $region) {
                if (!empty($translations['Intl'][$region])) {
                    $translations['Intl'][$region] = $motherland . '(' . $translations['Intl'][$region] . ')';
                }
            }
